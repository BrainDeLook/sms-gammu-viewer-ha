...
- 💬 **SMS в виде диалогов** — все сообщения от одного отправителя в одном чате, отправка и приём
- 📞 **Исходящие звонки** (только дозвон) — через отдельный голосовой AT-интерфейс на модемах с несколькими serial-портами (например Huawei)
- 🚪 **Сущности звонков** — `cover`/`button` которые звонят на фиксированный номер, для ворот и домофонов открывающихся по входящему звонку
- 📇 **Телефонная книга** — имена контактов отображаются в списке диалогов, истории звонков и шапке чата
- 🔔 **Push-уведомления** — при новом SMS с переходом по нажатию (iOS и Android), сгруппированы по отправителю
- 🩺 **Автовосстановление модема** — сбрасывает модем при повторных ошибках с предупреждением в статус-баре
- 📨 **Сенсоры** — `sensor.sms_unread_count`, `sensor.sms_last_sms_number`, `sensor.sms_last_sms_text` для автоматизаций
- 🌐 **Русский / English интерфейс** — переключается независимо от языка самой HA
- 🔍 **Поиск** по номеру и тексту сообщения
- 📱 **Адаптив для мобилок** — отзывчивый интерфейс с bottom sheet меню и навигацией назад
