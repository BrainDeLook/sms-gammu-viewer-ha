...
- 📞 **Исходящие голосовые звонки** (только дозвон) — через отдельный голосовой AT-интерфейс на модемах с несколькими serial-портами (например Huawei). Отдельная кнопка звонка или кнопка в чате, плюс сущность `notify.sms_gammu_call` для автоматизаций
