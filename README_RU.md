...
Нативная панель для **SMS-переписки и голосовых звонков** прямо в Home Assistant. Она предназначена для собственного аддона [SMS Modem Gateway](https://github.com/BrainDeLook/sms-modem-gateway-ha), который надёжно собирает многочастные SMS и передаёт готовые сообщения в Home Assistant через долговечную REST-очередь.
...
- 📌 **Закреплённые сообщения** — несколько закрепов, переход между ними и отдельный список закрепов
- 🗂️ **Папки чатов** — пользовательские папки и автоматические папки «Люди»/«Бренды»
- 👆 **Долгое нажатие** (мобиль) / **ПКМ** (десктоп) — копировать, закреплять, удалять и управлять папками
- 👈 **Свайпы** (мобиль) — влево: заглушить/удалить, вправо: прочитать/закрепить
- 📞 **Исходящие звонки** через AT-интерфейс
- 📖 **Профили контактов** — имя, фото, метка, e-mail, организация, день рождения, заметки и произвольные соцсети/способы связи
...
- Аддон [SMS Modem Gateway](https://github.com/BrainDeLook/sms-modem-gateway-ha)
...
### 1. Установи SMS Modem Gateway

1. Открой **Настройки → Дополнения → Магазин дополнений**
2. Открой ⋮ → **Репозитории**
3. Добавь `https://github.com/BrainDeLook/sms-modem-gateway-ha`
4. Установи **SMS Modem Gateway**
5. Выбери serial-устройство модема, задай логин и пароль, затем запусти аддон

Не запускай одновременно два SMS-шлюза на одном и том же порту модема.

### 2. Установи SMS Gammu Viewer через HACS (рекомендуется)
...
Некоторые USB GSM-модемы (особенно Huawei) выставляют **несколько serial-интерфейсов** через одно USB-подключение — обычно один для SMS, другой отдельно для голосовых AT-команд. В таком случае интеграция может дозваниваться напрямую через голосовой интерфейс, независимо от REST API SMS Modem Gateway, **без риска конфликта портов**.
...
1. Узнай путь к голосовому интерфейсу модема:
   ...
   ```
   Найди второй путь к устройству, отдельный от того, который использует SMS Modem Gateway.
...
SMS Modem Gateway (REST API :5000)
       ↓ долговечная очередь логических SMS + подтверждение по ID
...
SMS длиннее 160 символов (латиница) или 70 (кириллица) оператор разбивает на физические транспортные части. SMS Modem Gateway читает их без `gammu.LinkSMS`, разбирает стандартные 8-битные и 16-битные concat-UDH и ждёт точный непротиворечивый набор частей.

Перед удалением данных с модема шлюз сохраняет собранное логическое SMS и журнал очистки в SQLite. Затем он удаляет только те физические записи, у которых совпадает отпечаток расположения, и помещает готовое сообщение в долговечную REST-очередь с ID. SMS Gammu Viewer сначала сохраняет сообщение в собственную SQLite-базу и только потом подтверждает ID очереди. Поэтому повторные запросы безопасны при сетевых ошибках, перезапусках и задержке частей длинного SMS.
...
|---|---|
| [BrainDeLook/SMS Modem Gateway](https://github.com/BrainDeLook/sms-modem-gateway-ha) | ✅ Рекомендуется: долговечная очередь и надёжная сборка multipart UDH |
| Старые pajikos-совместимые шлюзы | ⚠️ Только режим совместимости, без тех же гарантий доставки |
...
- **[Gammu](https://wammu.eu/gammu/)** и **python-gammu** — низкоуровневое общение с модемом в SMS Modem Gateway. Сборку частей, хранение и доставку реализует наш собственный шлюз, а не `gammu.LinkSMS`.
