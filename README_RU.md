...
- **Отдельная кнопка звонка** — зелёная плавающая кнопка (📞) появляется рядом с «Новое сообщение» в сайдбаре после настройки голосового порта. Нажми — откроется меню с историей последних 30 звонков (с иконками результата) и полем для набора нового номера, открытый диалог не нужен.
- **Кнопка в чате** — также доступна в шапке открытой переписки.
- **Автоматизации** — вызови сервис `notify.sms_gammu_call` с номером телефона в качестве сообщения:
  ```yaml
  service: notify.sms_gammu_call
  data:
    message: "+79001234567"
  ```
  Можно перечислить несколько номеров через `|` — дозвон остановится на первом отвеченном.
